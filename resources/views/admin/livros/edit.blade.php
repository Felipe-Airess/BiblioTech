<x-guest-layout>
    <div class="py-6">
        ...
        <form method="POST" action="{{ route('livros.update', $livro->id) }}" enctype="multipart/form-data" class="font-inter space-y-6">
            @csrf
            @method('PUT')

            <div>
                <x-input-label for="titulo" value="Título do Livro" class="text-gray-300 font-semibold uppercase text-[10px] tracking-widest" />
                <x-text-input id="titulo" class="block mt-1 w-full bg-gray-900 border-gray-700 text-white focus:border-[#1E3A8A] focus:ring-[#1E3A8A] rounded-xl" type="text" name="titulo" :value="old('titulo', $livro->titulo)" required autofocus />
            ...
            <div>
                ...
                <x-text-input id="autor" class="block mt-1 w-full bg-gray-900 border-gray-700 text-white focus:border-[#1E3A8A] focus:ring-[#1E3A8A] rounded-xl" type="text" name="autor" :value="old('autor', $livro->autor)" required />
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <x-text-input id="categoria" class="block mt-1 w-full bg-gray-900 border-gray-700 text-white focus:border-[#1E3A8A] focus:ring-[#1E3A8A] rounded-xl" type="text" name="categoria" :value="old('categoria', $livro->categoria)" required />
                </div>
                
                <div>
                    <x-input-label for="quantidade" value="Estoque" class="text-gray-300 font-semibold uppercase text-[10px] tracking-widest" />
                    <x-text-input id="quantidade" class="block mt-1 w-full bg-gray-900 border-gray-700 text-white focus:border-[#1E3A8A] focus:ring-[#1E3A8A] rounded-xl" type="number" name="quantidade" min="0" :value="old('quantidade', $livro->quantidade)" required />
                </div>

                <div>
                    <x-input-label for="data_publicacao" value="Data Lançamento" class="text-gray-300 font-semibold uppercase text-[10px] tracking-widest" />
                    <x-text-input id="data_publicacao" class="block mt-1 w-full bg-gray-900 border-gray-700 text-white focus:border-[#1E3A8A] focus:ring-[#1E3A8A] rounded-xl" type="date" name="data_publicacao" :value="old('data_publicacao', $livro->data_publicacao ? $livro->data_publicacao->format('Y-m-d') : '')" required />
                </div>
            </div>
            ...
            <div class="flex flex-col md:flex-row items-end gap-4">
                <div class="w-full md:w-2/3">
                    ...
                    <x-text-input id="isbn" class="block mt-1 w-full bg-gray-900 border-gray-700 text-white focus:border-[#1E3A8A] focus:ring-[#1E3A8A] rounded-xl" type="text" name="isbn" :value="old('isbn', $livro->isbn)" required placeholder="000-00-000-0000-0" />
                </div>

                <div class="w-full md:w-1/3 pb-3 flex items-center bg-gray-900/50 p-3 rounded-xl border border-gray-800">
                    <input id="e_bestseller" type="checkbox" name="e_bestseller" value="1" {{ old('e_bestseller', $livro->e_bestseller) ? 'checked' : '' }} class="rounded border-gray-700 bg-gray-900 text-[#1E3A8A] shadow-sm focus:ring-[#1E3A8A]">
            ...
            <div>
                ...
                <textarea id="sinopse" name="sinopse" rows="3" class="block mt-1 w-full bg-gray-900 border-gray-700 text-white focus:border-[#1E3A8A] focus:ring-[#1E3A8A] rounded-xl shadow-sm">{{ old('sinopse', $livro->sinopse) }}</textarea>
            </div>

            <div class="bg-gray-900/50 p-4 rounded-xl border border-dashed border-gray-700">
                <x-input-label for="capa" value="Trocar Capa (Opcional)" class="text-gray-300 font-semibold uppercase text-[10px] tracking-widest mb-2" />
